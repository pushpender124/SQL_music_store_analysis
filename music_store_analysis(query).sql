...
GROUP BY 1,2,3   # customer.customer_id
...
FROM Music_Store.customer
JOIN Music_Store.invoice ON invoice.customer_id = customer.customer_id
JOIN Music_Store.invoice_line ON invoice_line.invoice_id = invoice.invoice_id
...
JOIN Music_Store.album ON album.album_id = track.album_id
JOIN Music_Store.artist ON artist.artist_id = album.artist_id
JOIN Music_Store.genre ON genre.genre_id = track.genre_id
WHERE genre.name LIKE 'Rock'
GROUP BY 1,2    # artist.artist_id
...
JOIN Music_Store.track t ON t.track_id = il.track_id
JOIN Music_Store.album alb ON alb.album_id = t.album_id
